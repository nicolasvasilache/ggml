...
void *align_with_offset(void *ptr, int offset) {
  ...
  return (char *)std::align(MAX_ALIGNMENT, MAX_ALIGNMENT, ptr, dummy_size) +
         offset;
}

void benchmark_function(size_t size, size_t q_size, int64_t iterations,
                        std::function<size_t(void)> function) {
  ...
  printf("      min cycles/%d vals   : %9.2f\n", QK,
         QK * min_time_cycles / (float)size);
  printf("      avg cycles/%d vals   : %9.2f\n", QK,
         QK * total_time_cycles / (float)(size * iterations));
...
void usage(char *argv[]) {
  ...
  printf(
      "  --size SIZE           set test size, divisible by 32 (L1_SIZE:%d)\n",
      L1_SIZE);
  printf("  -3                    use size as L1, L2, L3 sizes (L1:%d L2:%d "
         "L3:%d)\n",
         L1_SIZE, L2_SIZE, L3_SIZE);
  printf("  -4                    use size as L1, L2, L3, MEM sizes (L1:%d "
         "L2:%d L3:%d MEM:%d)\n",
         L1_SIZE, L2_SIZE, L3_SIZE, MEM_SIZE);
  printf("  --op OP               set test opration as "
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "--size") {
      ...
      } else if (op == "quantize_row_q,dequantize_row_q") {
        params.op_quantize_row_q = true;
  ...
  if (!(params.op_quantize_row_q_reference || params.op_quantize_row_q ||
        params.op_dequantize_row_q || params.op_quantize_row_q_dot ||
        params.op_vec_dot_q)) {
    params.op_quantize_row_q_reference = params.op_quantize_row_q =
        params.op_dequantize_row_q = params.op_quantize_row_q_dot =
            params.op_vec_dot_q = true;
  }
  ...
  float *test_data1 =
      (float *)align_with_offset(test_data1_v.data(), params.alignment_offset);
  float *test_data2 =
      (float *)align_with_offset(test_data2_v.data(), params.alignment_offset);
  float *test_q1 =
      (float *)align_with_offset(test_q1_v.data(), params.alignment_offset);
  float *test_q2 =
      (float *)align_with_offset(test_q2_v.data(), params.alignment_offset);
  float *test_out =
      (float *)align_with_offset(test_out_v.data(), params.alignment_offset);
  ...
  for (int i = 0; i < GGML_TYPE_COUNT; i++) {
    ggml_type type = (ggml_type)i;
    ggml_type_traits_t qfns = ggml_internal_get_type_traits(type);
    if (!params.include_types.empty() && ggml_type_name(type) &&
        std::find(params.include_types.begin(), params.include_types.end(),
                  ggml_type_name(type)) == params.include_types.end()) {
      continue;
    }
      ...
      if (params.op_quantize_row_q_reference) {
        ...
        for (size_t size : params.test_sizes) {
          printf("    %zu values (%.2f MB)\n", size,
                 4 * size / (float)(1024 * 1024));
          auto quantize_fn = [&](void) {
            qfns.from_float_reference(test_data1, test_q1, size);
            return test_q1[0];
          };
          size_t quantized_size =
              size / ggml_blck_size(type) * ggml_type_size(type);
...
#define DUMP_VALUES_FOR_COMPARISON 1
#if DUMP_VALUES_FOR_COMPARISON
      for (size_t size : params.test_sizes) {
        printf("\nIN: ");
        for (int i = 0; i < size * 4; ++i)
          printf("%x ", ((char *)test_data1)[i] & 0xff);
#define DUMP_INTERMEDIATE_QUANTIZATIONS 1
#if DUMP_INTERMEDIATE_QUANTIZATIONS
        printf("\nQ: ");
        for (int i = 0; i < size * 4; ++i)
          printf("%x ", ((char *)test_q1)[i] & 0xff);
#endif
        printf("\nOUT: ");
        for (int i = 0; i < size * 4; ++i)
          printf("%x ", ((char *)test_out)[i] & 0xff);
        printf("\n");
      }
#endif
