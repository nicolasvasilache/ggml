# objdump -d ./build-base/src/libggml.a --no-addresses --no-show-raw-insn -M att > /tmp/foo.s
# llvm-mca /tmp/foo.s -mcpu=cascadelake -iterations=100 -timeline 

# source ntv-scripts.sh && ggmlir-compile ../llvm-project/mlir/test/Dialect/Linalg/ggml-q5-1-reference.mlir 11 && test-quantize-perf base 32 > /tmp/q.base ; test-quantize-perf mlir 32 > /tmp/q.mlir && diff /tmp/q.base /tmp/q.mlir
...
# source ntv-scripts.sh &&  ggmlir-compile ../llvm-project/mlir/test/Dialect/Linalg/ggml-q5-1-reference.mlir 11 && for i in $(seq 0 10); do whisper-run base 1 2>&1 | grep -A5 "encode time = "  | grep -v prompt; done
